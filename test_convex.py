...
class TestVoid:
    ...
    # Периметр нульугольника внутри заданного прямоугольника равен нулю ()
    def test_per(self):
        assert self.f.per() == 0.0
...
class TestPoint:
    ...
    # Периметр одноугольника внутри заданного прямоугольника равен нулю ()
    def test_per(self):
        assert self.f.per() == 0.0
...
class TestSegment:
    ...
    # Периметр двуугольника внутри заданного прямоугольника ()
    def test_per1(self):
        assert self.f.per() == approx(2.0)

    # Двуугольник может быть вне заданного прямоугольника ()
    def test_per2(self):
        Figure.k = R2Point(-1.0, -1.0)
        Figure.m = R2Point(0.0, 0.0)
        self.f = Segment(R2Point(0.0, 0.0), R2Point(1.0, 0.0))
        assert self.f.per() == approx(0.0)
...
class TestPolygon:
    ...
    def test_per1(self):
        Figure.k = R2Point(-10.0, -10.0)
        Figure.m = R2Point(0.0, 10.0)
        self.f = Polygon(
            R2Point(
                0.0, 0.0), R2Point(
                10.0, 0.0), R2Point(
                0.0, 10.0))
        assert self.f.per() == approx(10.0)
    ...
    def test_per2(self):
        ...
        self.f = Polygon(
            R2Point(
                ...
                -7.0, 0.0), R2Point(
                -2.0, 0.0))
        assert self.f.per() == approx(16.216116)
